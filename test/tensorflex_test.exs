defmodule TensorflexTest do
  ...
  test "Tensor Functionalities Test" do
    ...
    assert {:ok, :tf_float} =
             Tensorflex.tensor_datatype(elem(Tensorflex.float32_tensor(vals, dims), 1))

    assert {:ok, :tf_float} =
             Tensorflex.tensor_datatype(elem(Tensorflex.float32_tensor_alloc(dims), 1))

    assert {:ok, :tf_float} =
             Tensorflex.tensor_datatype(elem(Tensorflex.float32_tensor(123.123), 1))

    assert {:ok, :tf_string} =
             Tensorflex.tensor_datatype(elem(Tensorflex.string_tensor("123.123"), 1))

    assert {:ok, :tf_double} =
             Tensorflex.tensor_datatype(elem(Tensorflex.float64_tensor(123.123), 1))
  ...
  test "Graph Loading Test" do
    assert [
             ...
             "add",
             "output"
           ] = Tensorflex.get_graph_ops(elem(Tensorflex.read_graph("./test/graphdef_toy.pb"), 1))
    ...
    assert [
             ...
             "Add_1",
             "output"
           ] = Tensorflex.get_graph_ops(elem(Tensorflex.read_graph("./test/graphdef_iris.pb"), 1))
  ...
  test "Matrex Session Test" do
    {:ok, graph} = Tensorflex.read_graph("./test/graphdef_toy.pb")

    in_vals = Matrex.new([[1.0, 1.0, 1.0], [2.0, 2.0, 2.0], [3.0, 3.0, 3.0]])
    {:ok, input_tensor} = Tensorflex.tensor_from_matrex(in_vals)
    assert Tensorflex.tensor_datatype(input_tensor) == {:ok, :tf_float}
